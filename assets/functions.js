...
module.exports = {
  ...
  // 乱数を元に武器をルーレットして返す
  outputRandomWeapon(repeatNumber, sourceArray) {
    let outputArray = []
  ...
  // 重複なしの武器ルーレット
  outputNoDuplicationRandomWeapon(repeatNumber, sourceArray) {
    let outputArray = []
    for(var i = 0; i < repeatNumber; i++) {
      let random = Math.floor(Math.random() * sourceArray.length)
  ...
  weaponDistribution(interaction) {
    ...
    if (weaponSelect === 'all') {
      // 武器種が全ての場合
      randomResultList = module.exports.outputRandomWeapon(vcMemberList.size, weapons.allWeapon)
      ...
      randomResultList = module.exports.outputRandomWeapon(vcMemberList.size, rouletteWeapon)
    } else {
      // 武器種指定の場合
      randomResultList = module.exports.outputRandomWeapon(
        vcMemberList.size,
